...
dp = 0.01 ###densidad inicial de pasto. Probabilidad de que una celda vacía (sin animal) empiece con pasto.
...
pp = 1 ###probabilidad de que en un turno dado el pasto crezca en una celda vacía adyacente.
...
def comienzo():
    ...
    for celda in snapshot:
        if celda[1] == 'None' or celda[1] == 'pasto':
            matriz.append([celda[0], celda[1]])
        else:
            matriz.append([celda[0], {
                "tipo": celda[1]['tipo'],
                "energia": celda[1]['energia'],
                "edad": celda[1]['edad']
            }])
...
def imprimir(turno):
    ...
    for celda in matriz:
        if celda[1] == 'None' or celda[1] == 'pasto':
            snapshot.append([celda[0], celda[1]])
        else:
            snapshot.append([celda[0], {
                "tipo": celda[1]["tipo"],
                "energia": celda[1]["energia"],
                "edad": celda[1]["edad"]
            }])
...
def __main__():
    ...
    for turno in range(tmax):
        ...
        for celda in matriz: #Luego de eso, primero se mueven todos los zorros y despues todos los conejos
            if str(type(celda[1])) == '<class \'dict\'>' and celda[1]['tipo'] == 'zorro' and celda[1]['movimiento'] != '':
                movimiento_zorro(celda)
            
        for celda in matriz: #Se mueven los conejos
            if str(type(celda[1])) == '<class \'dict\'>' and celda[1]['tipo'] == 'conejo' and celda[1]['movimiento'] != '':
